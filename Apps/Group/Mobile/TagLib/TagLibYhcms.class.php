...
class TagLibYhcms extends TagLib {
	protected $tags = array(
		...
		'cfg_mobile_logo'	=> array('close' => 0),//手机网站logo
		'cfg_mobile_powerby'	=> array('close' => 0),//手机
...
str;
	...
	public function _banner($attr, $content) {
		...
		$limit = $attr['limit'] == '' ? -1 : $attr['limit'];
		$str = <<<str
<?php
	\$_bannerlist = M('Banner_wap')->order('sort')->limit($limit)->select();
...
str;
	...
	//导航
	public function _navwaplist($attr, $content) {
		$attr = !empty($attr)? $this->parseXmlAttr($attr, 'navwaplist') : null;
		$str = <<<str
<?php
	\$_navwaplist = M('Nav_wap')->order('sort')->select();
	import('Class.Category', APP_PATH);	

	foreach(\$_navwaplist as \$autoindex => \$navwaplist):
		switch (\$navwaplist['navtype']) {
			case '1':
				\$navwaplist['ico'] = "fa-phone-square";
				\$navwaplist['url'] = "tel:".\$navwaplist['url'];
				break;
			case '2':
				\$navwaplist['ico'] = "fa-qq";
				\$navwaplist['url'] = "http://wpa.qq.com/msgrd?v=3&uin=".\$navwaplist['url']."&site=qq&menu=yes";
				break;
			case '3':
				\$navwaplist['ico'] = "fa-map-marker";
				\$navwaplist['url'] = \$navwaplist['url'];
				break;
			case '4':
				\$navwaplist['ico'] = "fa-home";
				\$navwaplist['url'] = C('cfg_weburl')."/Mobile/";
				break;
			case '5':
				if(strstr(\$navwaplist['url'], '@')){
					\$cdata = M('category')->find(str_replace('@', '', \$navwaplist['url']));
					\$navwaplist['url'] = C('cfg_weburl').getUrl(\$cdata);
				}else{
					\$navwaplist['url'] = C('cfg_weburl').\$navwaplist['url'];
				}
				\$navwaplist['ico'] = \$navwaplist['ico'];
				break;
		}
?>
str;
...
str;
	...
	public function _cfg_mobile_logo($attr, $content) {return C('cfg_mobile_logo');}//手机网站logo
	public function _cfg_mobile_powerby($attr, $content) {return C('cfg_mobile_powerby');}//手机网站底部版权
