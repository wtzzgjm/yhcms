...
class DatabaseAction extends CommonAction {
    ...
    public function delSqlFiles() {
        ...
        if ($batchFlag) {
            ...
            $files[] = I('sqlfilename' , '').'.sql';
        }

        if (empty($files)) {
            $this->ajaxReturn(0, "请选择要删除的sql文件!", 0);
        }
        foreach ($files as $file) {
            $jg = unlink($this->getDbPath(). '/' . $file);
        }

        $this->ajaxReturn(1, "已删除：" . implode(",", $files), 1);
    }
    ...
    public function repair() {
        ...
        if (empty($table)) {
            $this->ajaxReturn(0, "请选择要修复的表！", 0);
        }
        $strTable = implode(', ', $table);
        if (!M()->query("REPAIR TABLE {$strTable} ")) {
            $strTable = '';
        }
        $this->ajaxReturn(1, "修复表成功!", 1);
    }
